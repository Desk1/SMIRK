...
# clip_pixel_values(image_tensor, ...)    -> Tensor    # clamp to valid normalised range
# resize_img(img, image_resolution)       -> Tensor    # resize to (H,W) or square int
# crop_img(img, arch_name)                -> Tensor    # centre crop based on architecture
# crop_img_for_finetune(img, target_size) -> Tensor    # square centre crop by pixel size
# crop_and_resize(img, arch_name, res)    -> Tensor    # crop then resize (primary helper)
